// Package auth holds credential + session primitives. M0 ships the password
// hashing and hardened cookie defaults; Redis-backed session storage and the
// role model land in M1 (spec 5.9).
package auth
...
// params are the argon2id cost parameters (sane 2025 defaults).
type params struct {
	memory      uint32
	iterations  uint32
	parallelism uint8
